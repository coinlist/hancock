...
// Usage: go run sign_message.go <private_key> <public_key> <message>
func main() {
	if len(os.Args) != 4 {
		fmt.Println("Usage: sign_message <private_key> <public_key> <message>")
		os.Exit(1)
	}
	...
	originalPubKey := publicKey
	pubKeyDecoded, err := base64.StdEncoding.DecodeString(originalPubKey)
	if err != nil {
		panic(err)
	}
	pubKey := tmEd25519.PubKey([]byte(pubKeyDecoded))
	address := sdk.ConsAddress(pubKey.Address()).String()

	// Output results
	fmt.Println("Address:", address)
	fmt.Println("Public Key:", originalPubKey)
